...
# Flow from client secrets stores client information in JSON file, this gets the JSON file
CLIENT_ID = json.loads(
    # client_secrets.json is from google dev website! 
    # It's important to use the same APPname here as in Google website
...
def gconnect():
    ...
    if request.args.get('state') != login_session['state']:
        # If they don't match it replies ith an error indicating there's a missmatch between the tokens
    ...
    try:
        # Upgrade the authorization code into a credentials object generating one time code
